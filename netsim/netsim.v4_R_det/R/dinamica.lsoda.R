...
dinamica.lsoda<-function(t,y,parms)  {
...
targ<-target(y,R,M,N,EXT.IN,EXT.FUN,t)
...
if (act.fun=="linear")   targetT<-targ*(1-Xmin)+Xmin
	...
	#cat("\n t=",t, "\t", round(y,3) ,"\t",round(targetT,3))
	targetT<-(1/(1+exp(-alpha*(targ-theta))))*(1-Xmin)+Xmin
	#aus<-apply(M,1,sum)
	#targetT[which(aus==0)]<-targ[which(aus==0)]

       }

dydt<-k*(targetT-y)
cat(file=fp, "lsoda step (t = ", t, "): ", dydt, "\n")
