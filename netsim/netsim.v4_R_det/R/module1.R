...
MODULE1<-function(num1,Nrim,Mdiscr,h,h.new,Sin,Sout,STin, STout,Freq.in, Freq.out,max.con, Cf.c,toll)
## INPUT
# num1: maximum number of nodes allowed for modules 1 (integer)
...

{
 ...
 controllo<-0
       while (controllo==0)
        {max.g1<-min(num1,Nrim)
         Ng<-sampleB(seq(1,max.g1,1),1)
         mm<-MOD1(Ng, Cf.c,max.con)

         aus.p<-probmod(M=mm,h=h,Sin=Sin,Sout=Sout,STin=STin,STout=STout,Freq.in=Freq.in,Freq.out=Freq.out,toll=toll)
	 Pm1<-aus.p[[1]]
	 #Sc<-aus.p[[2]]
         if (!is.na(Pm1))   controllo<-1
          else {num1<-num1-1; if (num1<1) {controllo<-1; aus.p[[1]]<-Pm1<-NA}}
        }

aus.p[[5]]<-c(1) #hubs
cat(file=fp, "module1 output:\n\taus.p = ")
print_list(aus.p)
return(aus.p)
}
