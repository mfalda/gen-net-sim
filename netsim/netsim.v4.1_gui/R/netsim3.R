...
gz_ctrl_g <- function(widget, event, userData)
{
  ...
  if (is.na(num)) {
    ...
    message_dialog_g("the value must be greater than zero")
...
glbls_g <- list(
  ...
  list("stat_t", TESTO, "0.001", num_ctrl_g, "absolute difference under which the signal can be considered stationary"),
  list("stat_w", TESTO, "0", num_ctrl_g, "the percentage of consecutive sampling times during which the signal has to satisfy the stationarity hypothesis (zero to disable the stationarity check)"),
  # 66
...
{
  # "primo_g" mi dice se e' la prima volta che ci passo, nel qual caso devo prima creare le liste in cui memorizzera' i valori
  primo_g <- get("primo_g", interf_g)
  argom_g <- get("argom_g", interf_g)
  if (num != PARAM)
    ctrl <- get(glbls_g[[num]][[1]], interf_g)
  txt_lbl <- glbls_g[[num]][[1]]
  # parametri alfa, beta e lambda
  ...
  else if (gtkWidgetIsSensitive(ctrl)) { # testo modificabile
    if (glbls_g[[num]][[2]] == TESTO) {
      txt1 <- as.numeric(gtkEntryGetText(ctrl))
      if (primo_g)
        argom_g[[num1]] <- list("numero", sprintf('%3.3f', txt1), NULL)
      else
        argom_g[[num1]][[2]] <- sprintf('%3.3f', txt1)
    }
...
on_assistant_prepare_g <- function(widget, page, data)
{
  ...
  if (current_page == 1) {
    ...
    k <- 1
    arg <- c(NGENI, CONN, MAXR, GAMMA, IND, CF, NUMSUB, KAPPA, FPR_G, ACTF, ALPHA, ALPHA + 8, ALPHA + 2 * 8, ALPHA + 3 * 8, ALPHA + 4 * 8, ALPHA + 5 * 8, WP, PARAM, TIMES, STAT_T, STAT_W, METHOD, SIM, SAVE)
    table1 <- get("riep", interf_g)
    for (j in 0:1) {
      for (i in 0:11) { # 11 righe
        if (k <= length(arg)) {
...
create_page5_g <- function(assistant)
{
  ...
  entry31 <- crea_controllo_g(TIMES + 4, FALSE, TRUE)
  ...
  entry32 <- crea_controllo_g(TIMES + 5, FALSE, TRUE)
