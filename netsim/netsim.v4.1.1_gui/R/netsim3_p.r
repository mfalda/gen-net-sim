...
gz_ctrl_s <- function(widget, event, userData)
{
  ...
  if (is.na(num)) {
    ...
    message_dialog_s("the value must be greater or equal than zero")
...
riepilogo_s <- function(table1, num, num1, x, y)
{
  ...
  if (num == WEIGHTS) {
    ...
    if (glbls_s[[num]][[2]] == 1) {
      ...
      if (primo_s)
        argom_s[[num1]] <- list("numero", sprintf('%3.3f', as.double(txt1)), NULL)
      else
        argom_s[[num1]][[2]] <- sprintf('%3.3f', as.double(txt1))
    }
...
on_assistant_prepare_s <- function(widget, page, data)
{
  ...
  if (current_page == 6) {
    k <- 1
    arg <- c(WEIGHTS, RULES, FPR_S, ALPHA + 5 * 8, ALPHA + 3 * 8, ALPHA + 4 * 8, ALPHA + 2 * 8, ACTF, ALPHA, ALPHA + 8, TIMES, METHOD, EXTIN, EXTF, PARAM, KO, SAVE, SIM)
