...
void write_m_i(const char *nomefile, const MATRICEi *m)
{
	int i = 0, j;
	FILE *fp;
	...
	fp = fopen(nomefile, "w");
	if (!fp) {
		Rprintf("Cannot write file '%s'", nomefile);
		error("");
		return;
	}
...
void write_m_d(const char *nomefile, const MATRICEd *m)
{
	int i = 0, j;
	FILE *fp;
	...
	for (i = 1; i <= LENGTHm1_d(m); i++) {
		fprintf(fp, "%d", i);
		for (j = 1; j <= LENGTHm2_d(m); j++)
			fprintf(fp, "\t%.16g", ACCEDIm_d(m, i, j));
		fprintf(fp, "\n");
	}
	fclose(fp);
...
void write_vn_d(const char *nomefile, const VETTOREd *v, const char *nome)
{
	int i = 0;
	FILE *fp;
...
void write_mn_d(const char *nomefile, const MATRICEd *m, const char *nome)
{
	int i = 0;
	FILE *fp;
	...
	fp = fopen(nomefile, "w");
	if (!fp) {
		Rprintf("Cannot write file '%s'", nome);
		error("");
		return;
	}
	fprintf(fp, "\t%s", nome);
