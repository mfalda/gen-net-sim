...
#ifdef MDEBUG
	VETTOREi * _arrotonda_d(VETTOREi *ris, const VETTOREd *v, const char *nome, const char *nomefile, int linea)
#else
	VETTOREi * _arrotonda_d(VETTOREi *ris, const VETTOREd *v)
#endif
{
#ifdef VERSIONE_d
	int i;
...
#ifdef FDEBUG
		_CREAstr(tmp, "");
		g_string_printf(tmp, "ATTENZIONE (arrotonda_d, linea %s # %d): il vettore e` stato arrotondato da double a intero: possibile perdita di precisione nei calcoli!", nomefile, linea);
		warning(tmp->str);
		fprintf(fp_fdbg, tmp->str);
		_CANCELLAstr(tmp);
#endif
